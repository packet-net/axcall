...
The release builds a `.deb` per program for amd64, arm64 and armhf, alongside a `.tar.gz` per platform carrying the lot. One package per program is deliberate: the package is named after a program, so `apt install axcall` gives you axcall and nothing else, and a node that wants only `axsocks` is not made to take a terminal it will never run. The tarball is the other way round, and is the try-it-out path.

Each package puts its binary in `/usr/lib/<name>/` with `/usr/bin/<name>` a symlink to it, and installs beside it the native helper that `System.IO.Ports` needs, which a NativeAOT build cannot static-link and loads at run time from the directory the binary sits in. Without it no serial port opens at all. Each package carries its own copy; it is 15 kB, and a shared package to hold one small file would buy a dependency edge for nothing. The tarball holds a directory for the same reason, and the files in it have to stay together. Windows needs none of this, because `System.IO.Ports` calls Win32 directly there, but its download is still an archive, since one bare file cannot hold two programs.

The `axcall` package declares `Conflicts: ax25-apps` and `Replaces: ax25-apps`, because that package owns `/usr/bin/axcall` and `/usr/share/man/man1/axcall.1.gz`. Installing it therefore removes ax25-apps, which also takes `axlisten`, `ax25ipd`, `ax25rtd` and `ax25mond` with it; all four are kernel-stack tools that no longer function, so in practice it removes dead weight, but it is worth knowing before you install on a machine you have not finished migrating. The others make no such claim: each owns a path nothing else has ever owned.

`axtun` is Linux-only and is packaged and shipped only for Linux. It works through `/dev/net/tun`; macOS has `utun`, which is a different interface, and Windows needs a driver installed. A binary whose only possible output is "not supported on this platform" is worse than no binary, so the macOS and Windows downloads carry the other three.
